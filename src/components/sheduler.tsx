...
const getShiftsByDay = (staffData: StaffWithShifts[], searchQuery: string) => {
  const shiftsByDay: Record<
    string,
    { staff: StaffWithShifts; shift: StaffShift }[]
  > = {};
  ...
  days.forEach((day) => {
    ...
    staffData.forEach((staff) => {
      ...
      if (
        ...
        !staff.name.toLowerCase().includes(normalizedSearchQuery)
      ) {
        return;
      }
...
export type AccountRole = "staffmanager" | "staff";
...
export default function Scheduler({
  ...
  staffData: StaffWithShifts[];
  searchQuery: string;
}) {
  const shiftsByDay = getShiftsByDay(staffData, searchQuery);
  const hasVisibleShifts = Object.values(shiftsByDay).some(
                  ...
                  {shiftsByDay[day].length > 0 ? (
                    shiftsByDay[day].map(({ staff, shift }) => (
                      <Link
                        ...
                        href={`/${accountRole === "staff" ? accountRole : `${accountRole}/${staff.role.toLowerCase()}`}/details/${staff.staff_id}`}
