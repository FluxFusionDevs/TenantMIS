...
export default async function Page({ params }: { params: any }) {
  ...
  const res2 = await fetch(`${baseUrl}/staffmanager/getRequests?role=SECURITY&staffId=${staffId}`);
    ...
    return {
      ...
      content: (
        <div className="flex items-start justify-start">
          ...
          <div className="mx-8">
            <p className="font-bold text-2xl opacity-80">{request.complaints?.subject}</p>
            <p className="text-sm opacity-75">UID# {request.complaints?.complaint_id}</p>
